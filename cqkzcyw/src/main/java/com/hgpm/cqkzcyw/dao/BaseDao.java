...
public class BaseDao {
	...
	public List list(String hql, Object[] param, int start, int size, String order) {
		...
		Query query = getSession().createQuery(hql);
		setParameters(query, param);
		if (start >= 0) { // �?始页数不小于0才进行分�?
			if (size > 0) { // 每页限制数量大于0才进行分�?,不然无意�?
				query.setFirstResult(start * size);
				query.setMaxResults(size);
			} else { // 每页限制数量不大�?0�?,分页无意�?
	 ...
	 * @param order
	 *            排序语句,本方法会自动加上" order by "关键�?
	 * @return 以Map<String,Object>组成的列�?,可能返回null
	 */
